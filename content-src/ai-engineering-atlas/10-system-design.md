...
<!-- VERIFY: representative latency breakdown between vector search, cross-encoder rerank, and generation for a mid-size retrieval pipeline, quoted as a range from a named benchmark rather than the single illustrative split above. -->
...
<!-- VERIFY: typical requests-per-minute or tokens-per-minute rate limits for a hosted LLM API at a mid tier, quoted from a named provider's current published limits rather than stated as a fixed number here. -->
