...
Across the major providers that ratio has sat around five or six to one — output tokens cost several times what input tokens cost, which is what makes brevity the cheaper lever.
...
The saving is large where it applies: cached prefix tokens are typically billed at roughly a tenth of the uncached input rate, above a minimum prefix on the order of a thousand tokens.
