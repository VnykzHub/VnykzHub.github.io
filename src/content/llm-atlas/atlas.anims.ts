...
export const ATLAS_ANIMS: Record<string, AtlasAnim> = {
  token_pred:(ctx,w,h,t)=>{
    ...
    const toks=["The","quick","brown","fox","[?]"],cols=["#F0B84C","#F0B84C","#F0B84C","#F0B84C","#4A9E93"];
    const tw=60,th=32,g=10,sx=(w-toks.length*(tw+g))/2,y=22;
    toks.forEach((tok,i)=>{
      const pulse=i===4?Math.abs(Math.sin(t*2)):1;
      ctx.globalAlpha=pulse;
      ctx.fillStyle=i===4?"rgba(74,158,147,.18)":"rgba(240,184,76,.1)";
      ctx.strokeStyle=i===4?"#4A9E93":"#F0B84C44";ctx.lineWidth=i===4?2:1.5;
      rr(ctx,sx+i*(tw+g),y,tw,th,6);ctx.fill();ctx.stroke();
      ctx.fillStyle=i===4?"#4A9E93":"#D9D3CC";
    ...
    probs.forEach(([word,p],i)=>{
      ...
      ctx.fillStyle="rgba(74,158,147,.12)";ctx.fillRect(bx,by+i*24,128,18);
      ctx.fillStyle="#4A9E93";ctx.fillRect(bx,by+i*24,bw,18);
  ...
  param_scale:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const ms=[{n:"GPT-1\n117M",p:.117,c:"#8FCFC4"},{n:"BERT\n340M",p:.34,c:"#6FBCB0"},{n:"GPT-2\n1.5B",p:1.5,c:"#F0B84C"},{n:"GPT-3\n175B",p:175,c:"#C4703F"},{n:"PaLM\n540B",p:540,c:"#E0705C"},{n:"DSV3\n37B active",p:37,c:"#4A9E93"}];
  ...
  phases:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const ps=[{n:"Pre-training",s:"Trillions of tokens",c:"#F0B84C",e:"📚"},{n:"Fine-tuning",s:"RLHF / DPO / SFT",c:"#4A9E93",e:"🎯"},{n:"Inference",s:"Frozen weights",c:"#E8C77A",e:"⚡"}];
  ...
  w2v:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const pts=[{w:"king",x:.2,y:.22,c:"#F0B84C"},{w:"queen",x:.2,y:.58,c:"#C4703F"},{w:"man",x:.65,y:.22,c:"#F0B84C"},{w:"woman",x:.65,y:.58,c:"#C4703F"},{w:"river",x:.42,y:.82,c:"#4A9E93"},{w:"bank",x:.14,y:.82,c:"#E8C77A"}];
    ...
    if(r>.3){ctx.strokeStyle="#6FBCB055";ctx.lineWidth=1.5;ctx.setLineDash([4,4]);[[0,1],[2,3],[0,2],[1,3]].forEach(([a,b])=>{ctx.beginPath();ctx.moveTo(pts[a].x*w,pts[a].y*h);ctx.lineTo(pts[b].x*w,pts[b].y*h);ctx.stroke();});ctx.setLineDash([]);}
    pts.forEach(p=>{
  ...
  rnn:(ctx,w,h,t)=>{
    ...
    for(let i=0;i<n;i++){
      ...
      ctx.fillStyle=act?"rgba(240,184,76,.28)":"rgba(240,184,76,.06)";ctx.strokeStyle=act?"#F0B84C":"#3A342E";ctx.lineWidth=1.5;
      ctx.fill();ctx.stroke();
      ...
      ctx.fillStyle=act?"rgba(74,158,147,.2)":"rgba(74,158,147,.05)";ctx.strokeStyle=act?"#4A9E93":"#24211D";ctx.lineWidth=2;
      ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.fillStyle=act?"#4A9E93":"#24211D";ctx.font="bold 10px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";
      ctx.fillText("h"+i,x,y);ctx.textBaseline="alphabetic";
      if(act){ctx.strokeStyle="#F0B84C55";ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(x,y+48);ctx.lineTo(x,y+r+2);ctx.stroke();}
      if(i<n-1&&act){const nx=sx+(i+1)*(r*2+g);ctx.strokeStyle="#4A9E9366";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x+r,y);ctx.lineTo(nx-r,y);ctx.stroke();ctx.fillStyle="#4A9E9366";ctx.beginPath();ctx.moveTo(nx-r,y-4);ctx.lineTo(nx-r,y+4);ctx.lineTo(nx-r+8,y);ctx.fill();}
  ...
  lstm:(ctx,w,h,t)=>{
    ...
    ctx.fillStyle="rgba(240,184,76,.1)";ctx.strokeStyle="#F0B84C";ctx.lineWidth=2;rr(ctx,cx-rw,cy-rh,rw*2,rh*2,10);ctx.fill();ctx.stroke();
    ctx.fillStyle="#D9D3CC";ctx.font="bold 13px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("LSTM Cell",cx,cy-rh+20);
    const gates=[{n:"Forget",x:cx-65,c:"#E0705C",v:Math.abs(Math.sin(t))},{n:"Input",x:cx,c:"#4A9E93",v:Math.abs(Math.cos(t))},{n:"Output",x:cx+65,c:"#E8C77A",v:Math.abs(Math.sin(t+1))}];
    gates.forEach(g=>{
    ...
    ctx.strokeStyle="#6FBCB0";ctx.lineWidth=3;ctx.setLineDash([6,3]);ctx.beginPath();ctx.moveTo(cx-rw-18,cy-18);ctx.lineTo(cx+rw+18,cy-18);ctx.stroke();ctx.setLineDash([]);
    ctx.fillStyle="#6FBCB0";ctx.font="9.5px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("Cell State C_t — additive update path (gradients flow freely)",cx,cy-rh-8);
  ...
  seq2seq_bottle:(ctx,w,h,t)=>{
    ...
    ctx.strokeStyle="#E8C77A";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(ex+ew,my);ctx.lineTo(mx-14,my);ctx.stroke();ctx.beginPath();ctx.moveTo(mx+14,my);ctx.lineTo(dx,my);ctx.stroke();
    const ts=(t*.5)%1;
    ctx.fillStyle=`rgba(232,199,122,${.8-ts*.5})`;ctx.beginPath();ctx.arc(ex+ew+ts*(dx-ex-ew),my,4,0,Math.PI*2);ctx.fill();
  ...
  enc_dec:(ctx,w,h,t)=>{
    ...
    ctx.fillStyle="rgba(240,184,76,.1)";ctx.strokeStyle="#F0B84C";ctx.lineWidth=2;rr(ctx,ex,ey,ew,eh,12);ctx.fill();ctx.stroke();
    ctx.fillStyle="#F0B84C";ctx.font="bold 13px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("ENCODER",ex+ew/2,ey+24);
    ctx.fillStyle="#9D968E";ctx.font="10px 'Space Grotesk'";ctx.fillText("Bidirectional Self-Attn",ex+ew/2,ey+44);
    ["h₁","h₂","h₃","h₄"].forEach((hl,i)=>{const hx=ex+18+i*38,hy=ey+eh-46;ctx.fillStyle="rgba(240,184,76,.28)";ctx.strokeStyle="#F0B84C66";ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(hx,hy,14,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#D9D3CC";ctx.font="10px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(hl,hx,hy);ctx.textBaseline="alphabetic";});
    const anim=Math.sin(t*1.5)*.5+.5;
    ctx.strokeStyle=`rgba(74,158,147,${.4+anim*.6})`;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(ex+ew,h/2);ctx.lineTo(dx,h/2);ctx.stroke();
    ctx.fillStyle=`rgba(74,158,147,${.4+anim*.6})`;ctx.beginPath();ctx.moveTo(dx,h/2-5);ctx.lineTo(dx,h/2+5);ctx.lineTo(dx+8,h/2);ctx.fill();
    ctx.fillStyle="#4A9E93";ctx.font="9.5px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("cross-attn",ex+ew+(dx-ex-ew)/2,h/2-10);
  ...
  attn:(ctx,w,h,t)=>{
    ...
    for(let r=0;r<n;r++)for(let c=0;c<n;c++){
      ...
      ctx.fillStyle=`rgba(240,184,76,${v*.85})`;ctx.fillRect(mx+c*ms,my+r*ms,ms-1,ms-1);
      ctx.strokeStyle="rgba(240,184,76,.18)";ctx.lineWidth=1;ctx.strokeRect(mx+c*ms,my+r*ms,ms-1,ms-1);
  ...
  mha:(ctx,w,h,t)=>{
    ...
    const cs=["#F0B84C","#4A9E93","#E8C77A","#C4703F"],labs=["Syntax","Coref","Local","Semant"];
    for(let i=0;i<nh;i++){
  ...
  ffn:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const layers=[4,10,4],cs=["#F0B84C","#4A9E93","#6FBCB0"],lw=55,lh=h-56,lg=(w-layers.length*lw-40)/(layers.length-1),sx=20,labs=["d_model","4·d_model","d_model"];
    layers.forEach((n,li)=>{
    ...
    ctx.fillStyle="#E8C77A";ctx.font="bold 10px 'IBM Plex Mono'";ctx.textAlign="center";ctx.fillText("Activation",sx+(lw+lg)+lw/2,h-22);
  },
  bert:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
  ...
  causal_mask:(ctx,w,h,t)=>{
    ...
    for(let r=0;r<n;r++)for(let c=0;c<n;c++){
      ...
      ctx.globalAlpha=ts;
      ctx.fillStyle=isMasked?"rgba(224,112,92,.15)":"rgba(74,158,147,.15)";
      ctx.fillRect(sx+c*cs,sy+r*cs,cs-1,cs-1);
      ctx.strokeStyle=isMasked?"#E0705C40":"#4A9E9340";
      ctx.lineWidth=1;
      ctx.strokeRect(sx+c*cs,sy+r*cs,cs-1,cs-1);
      ctx.fillStyle=isMasked?"#E0705C":"#4A9E93";
  ...
  t5:(ctx,w,h,t)=>{
    ...
    ctx.fillStyle="rgba(240,184,76,.1)";ctx.strokeStyle="#F0B84C";ctx.lineWidth=2;rr(ctx,ex,ey,ew,eh,12);ctx.fill();ctx.stroke();
    ctx.fillStyle="#F0B84C";ctx.font="bold 13px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("ENCODER",ex+ew/2,ey+22);
    ["translate to German:","Hello world"].forEach((s,i)=>{const by=ey+40+i*30;rr(ctx,ex+8,by,ew-16,24,4);ctx.fillStyle=i===0?"rgba(232,199,122,.15)":"rgba(240,184,76,.18)";ctx.fill();ctx.strokeStyle=i===0?"#E8C77A40":"#F0B84C40";ctx.lineWidth=1;ctx.stroke();ctx.fillStyle="#D9D3CC";ctx.font="10px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(s,ex+ew/2,by+12);ctx.textBaseline="alphabetic";});
    const anim=Math.sin(t*1.5)*.5+.5;ctx.strokeStyle=`rgba(74,158,147,${.5+anim*.5})`;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(ex+ew,h/2);ctx.lineTo(dx,h/2);ctx.stroke();
  ...
  moe:(ctx,w,h,t)=>{
    ...
    for(let e=0;e<ne;e++){const ex=sx+e*(ew+eg),isA=an.includes(e);ctx.fillStyle=isA?"rgba(74,158,147,.2)":"rgba(26,23,20,.5)";ctx.strokeStyle=isA?"#4A9E93":"#3A342E";ctx.lineWidth=isA?2:1;rr(ctx,ex,ey,ew,eh,8);ctx.fill();ctx.stroke();ctx.fillStyle=isA?"#4A9E93":"#3A342E";ctx.font=(isA?"bold ":"")+"10.5px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("E"+(e+1),ex+ew/2,ey+eh/2);ctx.textBaseline="alphabetic";ctx.strokeStyle=isA?"rgba(74,158,147,.65)":"rgba(58,52,46,.35)";ctx.lineWidth=isA?2:1;ctx.beginPath();ctx.moveTo(tx,ry+24);ctx.lineTo(ex+ew/2,ey);ctx.stroke();}
  ...
  ssm:(ctx,w,h,t)=>{
    ...
    ctx.fillStyle="rgba(111,188,176,.14)";ctx.strokeStyle="#6FBCB0";ctx.lineWidth=2;rr(ctx,sx,sy,sw,sh,8);ctx.fill();ctx.stroke();
    ctx.fillStyle="#6FBCB0";ctx.font="bold 11.5px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("State x_k",sx+sw/2,sy+sh/2);ctx.textBaseline="alphabetic";
    const n=5,tw=38,th=24,iy=h-54,tg=8,isx=(w-n*(tw+tg))/2;
    for(let i=0;i<n;i++){const ix=isx+i*(tw+tg),act=i<=Math.floor(t*.55)%n;rr(ctx,ix,iy,tw,th,5);ctx.fillStyle=act?"rgba(240,184,76,.28)":"rgba(240,184,76,.06)";ctx.fill();ctx.strokeStyle=act?"#F0B84C":"#3A342E60";ctx.lineWidth=1.5;ctx.stroke();ctx.fillStyle=act?"#D9D3CC":"#3A342E";ctx.font="10px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("u"+i,ix+tw/2,iy+th/2);ctx.textBaseline="alphabetic";if(act){ctx.strokeStyle="#F0B84C55";ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(ix+tw/2,iy);ctx.lineTo(sx+sw/2,sy+sh);ctx.stroke();}}
    const my=sy+sh+52;rr(ctx,sx-38,my,sw+76,38,8);ctx.fillStyle="rgba(74,158,147,.1)";ctx.fill();ctx.strokeStyle="#4A9E9360";ctx.lineWidth=1.5;ctx.stroke();
    ctx.fillStyle="#4A9E93";ctx.font="bold 10.5px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("Mamba: B, C, Δ = f(input) — selective",sx+sw/2,my+13);ctx.fillStyle="#9D968E";ctx.font="9.5px 'Space Grotesk'";ctx.fillText("O(1) memory/step at inference — O(n) total",sx+sw/2,my+28);ctx.textBaseline="alphabetic";
  },
  linAttn:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
  ...
  rwkv:(ctx,w,h,t)=>{
    ...
    for(let i=0;i<n;i++){const x=sx+i*(r*2+g),decay=Math.exp(-(n-1-i)*.6);ctx.globalAlpha=decay*.9+.1;ctx.fillStyle=`rgba(111,188,176,${decay*.4})`;ctx.strokeStyle=`rgba(111,188,176,${decay*.8})`;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#6FBCB0";ctx.font="bold 9px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("v"+i,x,y);ctx.textBaseline="alphabetic";if(i===n-1){ctx.fillStyle="#9D968E";ctx.font="8.5px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("current",x,y+r+13);}ctx.globalAlpha=1;}
    const outX=w-52,outY=y;ctx.strokeStyle="#E8C77A";ctx.lineWidth=1.5;
    for(let i=0;i<n;i++){const x=sx+i*(r*2+g);ctx.beginPath();ctx.moveTo(x+r,y);ctx.lineTo(outX,outY);ctx.stroke();}
    ctx.fillStyle="rgba(232,199,122,.18)";ctx.strokeStyle="#E8C77A";ctx.lineWidth=2;ctx.beginPath();ctx.arc(outX,outY,20,0,Math.PI*2);ctx.fill();ctx.stroke();
    ctx.fillStyle="#E8C77A";ctx.font="bold 9px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("wkv_t",outX,outY);ctx.textBaseline="alphabetic";
  ...
  retnet:(ctx,w,h,t)=>{
    ...
    for(let i=0;i<n;i++){const x=sx+i*(r*2+g),act=i<=Math.floor(t*.5)%n;ctx.fillStyle=act?"rgba(74,158,147,.22)":"rgba(74,158,147,.05)";ctx.strokeStyle=act?"#4A9E93":"#24211D";ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle=act?"#4A9E93":"#24211D";ctx.font="bold 9px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("p"+i,x,y);ctx.textBaseline="alphabetic";}
    for(let i=0;i<n-1;i++){const x1=sx+i*(r*2+g)+r,x2=sx+(i+1)*(r*2+g)-r,dec=Math.pow(gamma,n-2-i);ctx.strokeStyle=`rgba(74,158,147,${dec*.8})`;ctx.lineWidth=1+dec;ctx.beginPath();ctx.moveTo(x1,y);ctx.lineTo(x2,y);ctx.stroke();ctx.fillStyle="#9D968E";ctx.font="8px 'IBM Plex Mono'";ctx.textAlign="center";ctx.fillText("γ^"+(n-2-i),x1+(x2-x1)/2,y-r-4);}
    const my=y+r+36;ctx.fillStyle="rgba(111,188,176,.1)";ctx.strokeStyle="#6FBCB060";ctx.lineWidth=1.5;rr(ctx,28,my,w-56,40,8);ctx.fill();ctx.stroke();
    ctx.fillStyle="#6FBCB0";ctx.font="bold 10.5px 'IBM Plex Mono'";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("γ="+gamma+" retention rate — 3 equivalent forms",w/2,my+13);ctx.fillStyle="#9D968E";ctx.font="9.5px 'Space Grotesk'";ctx.fillText("Parallel (train)  ·  Recurrent (inference)  ·  Chunk-wise",w/2,my+28);ctx.textBaseline="alphabetic";
  },
  toktradeoff:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const apps=[{n:"Characters",toks:["u","n","h","a","p","p","y"],c:"#E0705C",v:"26 vocab"},{n:"BPE",toks:["un","happy"],c:"#4A9E93",v:"50K vocab"},{n:"WordPiece",toks:["un","##happy"],c:"#E8C77A",v:"30K vocab"},{n:"Full Word",toks:["unhappy"],c:"#F0B84C",v:"∞ vocab"}];
  ...
  wordpiece:(ctx,w,h,t)=>{
    ...
    rows.forEach((row,ri)=>{
      ...
      row.bpe.forEach(tok=>{const cw_=tok.length*7+10;rr(ctx,bx,ry+20,cw_,18,3);ctx.fillStyle="rgba(240,184,76,.15)";ctx.fill();ctx.strokeStyle="#F0B84C40";ctx.lineWidth=1;ctx.stroke();ctx.fillStyle="#9D968E";ctx.font="10px 'IBM Plex Mono'";ctx.textAlign="left";ctx.textBaseline="middle";ctx.fillText(tok,bx+4,ry+29);ctx.textBaseline="alphabetic";bx+=cw_+4;});
      bx+=14;ctx.fillStyle="#9D968E";ctx.font="9px 'Space Grotesk'";ctx.fillText("WP:",bx,ry+30);bx+=30;
      row.wp.forEach(tok=>{const cw_=tok.length*7+10;rr(ctx,bx,ry+20,cw_,18,3);ctx.fillStyle="rgba(74,158,147,.15)";ctx.fill();ctx.strokeStyle="#4A9E9340";ctx.lineWidth=1;ctx.stroke();ctx.fillStyle="#4A9E93";ctx.font="10px 'IBM Plex Mono'";ctx.textAlign="left";ctx.textBaseline="middle";ctx.fillText(tok,bx+4,ry+29);ctx.textBaseline="alphabetic";bx+=cw_+4;});
    });
  },
  sp:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    ...
    let y=drawRow(sp,"SentencePiece (▁=word boundary, language-agnostic):",28,"#4A9E93");
    const byteArr=[...(new TextEncoder().encode(text))].map(b=>b.toString());
    drawRow(byteArr.slice(0,18),"Byte-level (UTF-8, vocab=256, zero unknowns):",y+6,"#6FBCB0");
  ...
  acts:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const fns=[{n:"ReLU",fn:x=>Math.max(0,x),c:"#E0705C"},{n:"GELU",fn:x=>x*.5*(1+Math.tanh(Math.sqrt(2/Math.PI)*(x+.044715*x*x*x))),c:"#4A9E93"},{n:"SwiGLU",fn:x=>x*(1/(1+Math.exp(-x)))*(Math.max(0,x)*.7+.5),c:"#6FBCB0"}];
  ...
  norm:(ctx,w,h,t)=>{
    ...
    drawDist(w*.76,h*.52,0,.5,"#4A9E93","Stable: σ=0.5");
    ctx.globalAlpha=1;ctx.strokeStyle="#6FBCB0";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(w*.36,h*.5);ctx.lineTo(w*.62,h*.5);ctx.stroke();ctx.fillStyle="#6FBCB0";ctx.beginPath();ctx.moveTo(w*.62,h*.5-5);ctx.lineTo(w*.62,h*.5+5);ctx.lineTo(w*.62+8,h*.5);ctx.fill();
    ctx.fillStyle="#6FBCB0";ctx.font="bold 10px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText("Norm (LayerNorm / RMSNorm)",w/2,h*.5-12);
  ...
  rope:(ctx,w,h,t)=>{
    ...
    const pos=[0,4,8,14],cs=["#F0B84C","#4A9E93","#E8C77A","#C4703F"],theta=.25;
  ...
  o2:(ctx,w,h,t)=>{
    ...
    lens.forEach((n,i)=>{const x=sx+i*(bw+g),lh=(n/maxN)*maxH*.42*Math.min(t,1),qh=(n*n/(maxN*maxN))*maxH*Math.min(t,1);ctx.fillStyle="rgba(74,158,147,.68)";ctx.fillRect(x,h-46-lh,bw/2-1,lh);ctx.fillStyle="rgba(224,112,92,.68)";ctx.fillRect(x+bw/2+1,h-46-qh,bw/2-1,qh);ctx.fillStyle="#9D968E";ctx.font="9px 'IBM Plex Mono'";ctx.textAlign="center";ctx.fillText(n+"K",x+bw/2,h-28);});
    ctx.fillStyle="#4A9E93";ctx.fillRect(sx,h-16,14,9);ctx.fillStyle="#E8E2D9";ctx.font="9.5px 'Space Grotesk'";ctx.textAlign="left";ctx.fillText("KV-cache O(n)",sx+18,h-9);
    ctx.fillStyle="#E0705C";ctx.fillRect(sx+132,h-16,14,9);ctx.fillText("Attn matrix O(n²)",sx+150,h-9);
  },
  ruler:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const models=[{n:"GPT-4",s:[99,97,91,84],c:"#4A9E93"},{n:"Llama3-70B",s:[98,93,84,68],c:"#F0B84C"},{n:"Mistral-L",s:[97,87,74,55],c:"#E8C77A"},{n:"Model-X",s:[95,72,48,22],c:"#E0705C"}];
  ...
  ssa:(ctx,w,h,t)=>{
    ...
    ctx.strokeStyle="#4A9E93";ctx.lineWidth=2.5;ctx.beginPath();lens.forEach((n,i)=>{const x=sx+i*cw/(lens.length-1),y=sy+ch*(1-Math.min(n*Math.log2(n+1)*.022/maxV,1));i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.stroke();ctx.globalAlpha=1;
  ...
  datawall:(ctx,w,h,t)=>{
    ...
    const a=Math.min(t*.5,1);ctx.globalAlpha=a;
    ctx.fillStyle="rgba(240,184,76,.18)";ctx.beginPath();ctx.moveTo(sx,sy+ch);yrs.forEach(y=>{const x=sx+((y-2020)/10)*cw,frac=Math.min((y-2020)/8,1);ctx.lineTo(x,sy+ch-frac*ch);});ctx.lineTo(sx+cw,sy+ch);ctx.closePath();ctx.fill();
    ctx.strokeStyle="#F0B84C";ctx.lineWidth=2.5;ctx.beginPath();yrs.forEach((y,i)=>{const x=sx+((y-2020)/10)*cw,frac=Math.min((y-2020)/8,1);i===0?ctx.moveTo(x,sy+ch-frac*ch):ctx.lineTo(x,sy+ch-frac*ch);});ctx.stroke();ctx.globalAlpha=1;
  ...
  scaling:(ctx,w,h,t)=>{
    ...
    const a=Math.min(t*.55,1);ctx.strokeStyle="#F0B84C";ctx.lineWidth=3;ctx.globalAlpha=a;ctx.beginPath();
  ...
  chinchilla:(ctx,w,h,t)=>{
    ...
    ctx.strokeStyle="#F0B84C";ctx.lineWidth=2.5;ctx.beginPath();for(let n=1;n<=maxN;n++){const x=sx+n/maxN*cw,y=sy+ch*(1-n/maxN);n===1?ctx.moveTo(x,y):ctx.lineTo(x,y);}ctx.stroke();
    ctx.strokeStyle="#4A9E93";ctx.lineWidth=2.5;ctx.setLineDash([6,4]);ctx.beginPath();for(let n=1;n<=maxN;n++){const d=20*n,x=sx+n/maxN*cw,y=sy+ch*(1-Math.min(d/(20*maxN),1));n===1?ctx.moveTo(x,y):ctx.lineTo(x,y);}ctx.stroke();ctx.setLineDash([]);ctx.globalAlpha=1;
    [{n:"GPT-3",nm:175,dm:300,c:"#E0705C"},{n:"Chinchilla",nm:70,dm:1400,c:"#4A9E93"}].forEach(m=>{const x=sx+m.nm/maxN*cw,y=sy+ch*(1-Math.min(m.dm/(20*maxN),1));ctx.fillStyle=m.c+"44";ctx.strokeStyle=m.c;ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,7,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#E8E2D9";ctx.font="bold 9px 'Space Grotesk'";ctx.textAlign="center";ctx.fillText(m.n,x,y-12);});
    ctx.fillStyle="#F0B84C";ctx.font="9.5px 'Space Grotesk'";ctx.textAlign="left";ctx.fillText("N (model size) →",sx,sy+ch+15);ctx.fillStyle="#4A9E93";ctx.textAlign="right";ctx.fillText("D* = 20·N →",sx+cw,sy+ch+15);
  },
  timeline:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const ms=[{n:"GPT-1",y:2018,p:.117,c:"#8FCFC4"},{n:"GPT-3",y:2020,p:175,c:"#F0B84C"},{n:"PaLM",y:2022,p:540,c:"#C4703F"},{n:"Mixtral",y:2023.5,p:46.7,act:12.9,c:"#4A9E93"},{n:"DSV3",y:2025,p:671,act:37,c:"#E8C77A"}];
    ...
    ctx.fillStyle="#F0B84C";ctx.fillRect(sx+cw-175,sy+8,10,10);ctx.fillStyle="#9D968E";ctx.font="9px 'Space Grotesk'";ctx.textAlign="left";ctx.fillText("Dense",sx+cw-162,sy+17);ctx.fillStyle="#4A9E93";ctx.fillRect(sx+cw-175,sy+24,10,10);ctx.fillText("MoE (dot=active params)",sx+cw-162,sy+33);
  },
  params:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const comps=[{n:"W_Q  (d²)",s:1,c:"#F0B84C"},{n:"W_K  (d²)",s:1,c:"#F0B84C88"},{n:"W_V  (d²)",s:1,c:"#F0B84CAA"},{n:"W_O  (d²)",s:1,c:"#F0B84C66"},{n:"FFN W1  (4d²)",s:4,c:"#4A9E93"},{n:"FFN W2  (4d²)",s:4,c:"#4A9E9388"},{n:"Norms (tiny)",s:.12,c:"#E8C77A"}];
  ...
  roofline:(ctx,w,h,t)=>{
    ...
    const rX = sx + cw*0.4;
    const rY = sy + ch*0.2;
    ctx.strokeStyle="#4A9E93";ctx.lineWidth=2;
    ...
    ctx.fillStyle="#F0B84C";ctx.beginPath();ctx.arc(pX,pY,6,0,Math.PI*2);ctx.fill();
  ...
  specdecode:(ctx,w,h,t)=>{
    ctx.clearRect(0,0,w,h);
    const draftColor = "#F0B84C", targetColor = "#4A9E93", errorColor = "#E0705C";
    const ts = (t*0.8)%1;
  ...
  sae:(ctx,w,h,t)=>{
    ...
    for(let i=0; i<nF; i++) {
       ...
       ctx.strokeStyle=`rgba(74,158,147,${alpha*0.5})`; ctx.lineWidth=1;
       ctx.beginPath();ctx.moveTo(w/2, y1+15);ctx.lineTo(fx, y3-15);ctx.stroke();
       ctx.fillStyle=`rgba(74,158,147,${alpha})`;
       ctx.beginPath();ctx.arc(fx, y3, 10, 0, Math.PI*2);ctx.fill();
       if (isActive) {
           ctx.fillStyle="#4A9E93"; ctx.font="9px 'IBM Plex Mono'";
...
,
    ...
    for(let i=0; i<cells; i++){
      for(let j=0; j<cells; j++){
        if(j > i) {
          ...
          ctx.fillStyle = 'rgba(74,158,147,'+(0.2 + (i===cells-1 && j===Math.floor((t*4)%cells) ? 0.5 : 0))+')'; // active
        }
  ...
  anim_mha_split: (ctx,w,h,t) => {
    ...
    for(let i=0; i<4; i++) {
      let y = -45 + i*30;
      ctx.fillStyle = ['#E0705C','#4A9E93','#E8C77A','#6FBCB0'][i];
  ...
  anim_gqa: (ctx,w,h,t) => {
    ...
    for(let i=0; i<2; i++){
      let kvy = -25 + i*60;
      ctx.fillStyle = '#4A9E93'; ctx.fillRect(20, kvy, 15, 20); // K
      ctx.fillStyle = '#E8C77A'; ctx.fillRect(37, kvy, 15, 20); // V
    }
  ...
  anim_cross_attn: (ctx,w,h,t) => {
    ...
    // Image KV
    ctx.fillStyle = '#4A9E93'; ctx.fillRect(30, -30, 20, 60);
    ctx.fillStyle = '#E8C77A'; ctx.fillRect(52, -30, 20, 60);
  ...
  anim_sparse_window: (ctx,w,h,t) => {
    ...
    for(let i=0; i<cells; i++){
      for(let j=0; j<cells; j++){
        ...
        if(active) {
          ctx.fillStyle = 'rgba(74,158,147,'+(0.3 + Math.sin(t*3 + i*0.5)*0.2)+')';
          ctx.fillRect(j*cs, i*cs, cs-1, cs-1);
        }
      }
    }
  ...
  anim_linear_kernel: (ctx,w,h,t) => {
    ...
    // O(n) visual
    ctx.fillStyle = 'rgba(74,158,147,0.3)';
    ctx.strokeStyle = '#4A9E93';
    let w2 = 10, h2 = 40;
