...
Vision transformers divide images into patches — typically 14×14 or 16×16 pixel squares — and project each patch into a token embedding, as if they were words. A 1024×1024 image becomes roughly 50 tokens; a PDF page at 150 dpi produces approximately 576 tokens. <!-- VERIFY: typical patch sizes and token counts for standard image resolutions; check Claude Vision documentation or a recent multimodal model technical report --> The model then processes these image tokens and text tokens through the same attention layers, so an image can appear anywhere in the context and be reasoned over like any other semantic unit.
