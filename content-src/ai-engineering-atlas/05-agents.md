...
<!-- VERIFY: cite a published long-horizon agentic benchmark showing success rate declining with step count, naming the benchmark and the model class it tested. -->
...
<!-- VERIFY: quote a specific prompt-caching discount and cache-lifetime figure from a named provider's current pricing page rather than a general claim, since both are provider- and model-specific and change without notice. -->
...
<!-- VERIFY: cite a source-backed range for typical added latency per agent tool-call round trip, since it depends heavily on model, prompt length, and the tool's own execution time. -->
